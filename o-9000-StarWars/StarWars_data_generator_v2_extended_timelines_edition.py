...
class StarWarsDataGenerator:
    """
    V2 of the data generator for out Star Wars ontology (still based on Episodes 4-6, but gone bonkers with events)...
    Creates realistic-looking data objects and relationships.
    Has waaaay too many events now, like 50-ish! per movie...
    Wookieepedia FTW!
    """

    def __init__(self, seed=None):
        """Initialize the generator with optional seed for reproducibility"""
        # what are the chances that we'll still mesh Life, Universe and Everything with Star Wars?
        # what are the chances that the seed will be 42?
        # Douglas Adams FTW!
        if seed:
            random.seed(seed)
    ...
    def generate_episode_4_timeline(self) -> List[Dict[str, Any]]:
        """Generate a detailed timeline of events from Episode 4: A New Hope (at least 21 events)"""
        timeline = []
        ...
        biggs = self._find_or_create_character("Biggs Darklighter")
        wedge = self._find_or_create_character("Wedge Antilles")
        red_leader = self._find_or_create_character("Garven Dreis")
        gold_leader = self._find_or_create_character("Jon Vander")
        greedo = self._find_or_create_character("Greedo")
        owen = self._find_or_create_character("Owen Lars")
        beru = self._find_or_create_character("Beru Lars")
        jawas = self._find_or_create_character("Jawas")
        tusken = self._find_or_create_character("Tusken Raiders")
        ...
        mos_eisley = self._find_or_create_planet(
            "Mos Eisley", "Desert", "Tatoo")
        ...
        tie_fighter = self._find_or_create_ship("TIE Fighter")
        star_destroyer = self._find_or_create_ship("Imperial Star Destroyer")

        # Timeline events -  adding more events, we can make a richer timelines here.
        # not fucking with the already working code, my brain's going cross-wired at this point
        # so this in-elegant nonsense is what we'll all have to live with.
        # don't like it? fork-it, fix-it, pull-request-it, technologic...

        # Opening sequence
        timeline.append(self._create_event(
            "Tantive IV is pursued by an Imperial Star Destroyer",
            "0 BBY",
            "Medium",
            participants=[leia["characterID"], vader["characterID"]],
            location=tantive["shipID"]
        ))

        timeline.append(self._create_event(
            "Imperial forces board the Tantive IV",
            "0 BBY",
            "Medium",
            participants=[vader["characterID"], leia["characterID"]],
            location=tantive["shipID"],
            quote="Where are those transmissions you intercepted?"
        ))
        ...
        timeline.append(self._create_event(
            "Darth Vader captures Princess Leia",
            "0 BBY",
            "High",
            participants=[vader["characterID"], leia["characterID"]],
            location=tantive["shipID"]
        ...
        timeline.append(self._create_event(
            "R2-D2 and C-3PO escape to Tatooine in a pod",
            "0 BBY",
            "Medium",
        ...
        # Tatooine sequence
        timeline.append(self._create_event(
            "R2-D2 and C-3PO are captured by Jawas",
            "0 BBY",
            "Low",
            participants=[r2d2["characterID"],
                          c3po["characterID"], jawas["characterID"]],
            location=tatooine["planetID"]
        ))

        timeline.append(self._create_event(
            "Owen Lars purchases R2-D2 and C-3PO from Jawas",
            "0 BBY",
            "Medium",
            participants=[owen["characterID"], beru["characterID"],
                          r2d2["characterID"], c3po["characterID"], luke["characterID"]],
            location=tatooine["planetID"]
        ))

        timeline.append(self._create_event(
            "R2-D2 escapes to find Obi-Wan Kenobi",
            "0 BBY",
            "Medium",
            participants=[r2d2["characterID"], luke["characterID"]],
            location=tatooine["planetID"]
        ))

        timeline.append(self._create_event(
            "Luke is attacked by Tusken Raiders while searching for R2-D2",
            "0 BBY",
            "Medium",
            participants=[luke["characterID"], tusken["characterID"]],
            location=tatooine["planetID"]
        ))
        ...
        timeline.append(self._create_event(
            "Luke Skywalker discovers Obi-Wan Kenobi's message from Princess Leia",
            "0 BBY",
            "High",
            participants=[luke["characterID"], obi_wan["characterID"],
                          r2d2["characterID"], leia["characterID"]],
            location=tatooine["planetID"]
        ))
        ...
        timeline.append(self._create_event(
            ...
            participants=[luke["characterID"], obi_wan["characterID"]],
            location=tatooine["planetID"],
            quote="Your father's lightsaber. This is the weapon of a Jedi Knight."
        ))

        timeline.append(self._create_event(
            "Imperial Stormtroopers murder Owen and Beru Lars",
            "0 BBY",
            "High",
            participants=[owen["characterID"], beru["characterID"]],
            location=tatooine["planetID"]
        ...
        timeline.append(self._create_event(
            "Luke decides to go with Obi-Wan to Alderaan",
            "0 BBY",
            "High",
            participants=[luke["characterID"], obi_wan["characterID"]],
            location=tatooine["planetID"],
            quote="I want to learn the ways of the Force and become a Jedi like my father."
        ))

        timeline.append(self._create_event(
            "Obi-Wan and Luke hire Han Solo in Mos Eisley Cantina",
            "0 BBY",
            "High",
            participants=[luke["characterID"], obi_wan["characterID"],
                          han["characterID"], chewie["characterID"]],
            location=mos_eisley["planetID"]
        ))

        timeline.append(self._create_event(
            "Han Solo kills Greedo in the Cantina",
            "0 BBY",
            "Medium",
            participants=[han["characterID"], greedo["characterID"]],
            location=mos_eisley["planetID"],
            quote="Sorry about the mess."
        ))

        timeline.append(self._create_event(
            "Millennium Falcon escapes from Tatooine",
            "0 BBY",
            "Medium",
            participants=[han["characterID"], chewie["characterID"],
                          luke["characterID"], obi_wan["characterID"]],
            location=falcon["shipID"]
        ))

        # Alderaan/Death Star sequence
        timeline.append(self._create_event(
            "The Empire tortures Princess Leia for information",
            "0 BBY",
            "Medium",
            participants=[leia["characterID"], vader["characterID"]],
        ...
        timeline.append(self._create_event(
            "Millennium Falcon is captured by the Death Star",
            "0 BBY",
            "Medium",
            participants=[han["characterID"], luke["characterID"],
                          obi_wan["characterID"], chewie["characterID"]],
            location=death_star["planetID"]
        ))

        timeline.append(self._create_event(
            "Obi-Wan Kenobi disables the Death Star's tractor beam",
            "0 BBY",
            "High",
        ...
        timeline.append(self._create_event(
            ...
            participants=[luke["characterID"], han["characterID"],
                          chewie["characterID"], leia["characterID"]],
            location=death_star["planetID"],
            quote="Aren't you a little short for a stormtrooper?"
        ))

        timeline.append(self._create_event(
            "Heroes escape through the Death Star's garbage compactor",
            "0 BBY",
            "Medium",
            participants=[luke["characterID"], han["characterID"],
                          leia["characterID"], chewie["characterID"]],
            location=death_star["planetID"],
            quote="One thing's for sure, we're all gonna be a lot thinner!"
        ))
        ...
        timeline.append(self._create_event(
            ...
            "0 BBY",
            "High",
            participants=[obi_wan["characterID"], vader["characterID"]],
            location=death_star["planetID"],
        ...
        timeline.append(self._create_event(
            "Heroes escape the Death Star in the Millennium Falcon",
            "0 BBY",
            "Medium",
            participants=[luke["characterID"], han["characterID"],
                          leia["characterID"], chewie["characterID"]],
        ...
        # Battle of Yavin sequence
        timeline.append(self._create_event(
            "Rebels analyze Death Star plans at Yavin 4 base",
            "0 BBY",
            "High",
            participants=[luke["characterID"], leia["characterID"]],
            location=yavin["planetID"]
        ))

        timeline.append(self._create_event(
            "Luke reunites with Biggs Darklighter at Rebel base",
            "0 BBY",
            "Low",
            participants=[luke["characterID"], biggs["characterID"]],
            location=yavin["planetID"]
        ))
        ...
        timeline.append(self._create_event(
            ...
            "0 BBY",
            "High",
            participants=[tarkin["characterID"], vader["characterID"]],
            location=death_star["planetID"],
            quote="You're all clear, kid! Now let's blow this thing and go home!"
        ))

        timeline.append(self._create_event(
            "Battle of Yavin begins",
            "0 BBY",
            "High",
            participants=[luke["characterID"], biggs["characterID"], wedge["characterID"],
                          red_leader["characterID"], gold_leader["characterID"]],
            location=yavin["planetID"],
            battle="Battle of Yavin"
        ))

        timeline.append(self._create_event(
            "Darth Vader joins the space battle in his TIE fighter",
            "0 BBY",
            "Medium",
            participants=[vader["characterID"]],
            location=tie_fighter["shipID"],
            battle="Battle of Yavin"
        ))

        timeline.append(self._create_event(
            "Han Solo returns to help Luke during the trench run",
            "0 BBY",
            "High",
            participants=[han["characterID"], chewie["characterID"],
                          vader["characterID"], luke["characterID"]],
        ...
        timeline.append(self._create_event(
            "Luke uses the Force to guide his torpedo shot",
            "0 BBY",
            "High",
            participants=[luke["characterID"], obi_wan["characterID"]],
            location=death_star["planetID"],
        ...
        timeline.append(self._create_event(
            "Rebels hold victory ceremony on Yavin 4",
            "0 BBY",
            "High",
            participants=[luke["characterID"], han["characterID"], leia["characterID"],
                          chewie["characterID"], r2d2["characterID"], c3po["characterID"]],
            location=yavin["planetID"]
        ))
    ...
    def generate_episode_5_timeline(self) -> List[Dict[str, Any]]:
        """Generate a detailed timeline of events from Episode 5: The Empire Strikes Back (at least 21 events)"""
        timeline = []
        ...
        c3po = self._find_or_create_droid("C-3PO")
        r2d2 = self._find_or_create_droid("R2-D2")
        boba_fett = self._find_or_create_character("Boba Fett")
        piett = self._find_or_create_character("Admiral Piett")
        ozzel = self._find_or_create_character("Admiral Ozzel")
        needa = self._find_or_create_character("Captain Needa")
        veers = self._find_or_create_character("General Veers")
        wampa = self._find_or_create_character("Wampa")
        rebel_officer = self._find_or_create_character("Rebel Officer")
        ...
        asteroid_field = self._find_or_create_planet(
            "Asteroid Field", "Asteroid", "Hoth Sector")
        echo_base = self._find_or_create_planet("Echo Base", "Frozen", "Hoth")
        ...
        x_wing = self._find_or_create_ship("X-Wing Red Five")
        tie_fighter = self._find_or_create_ship("TIE Fighter")
        slave_one = self._find_or_create_ship("Slave I")
        snowspeeder = self._find_or_create_ship("T-47 Snowspeeder")

        # Timeline events - More timeline events motherfuckerz!

        # Hoth sequence
        timeline.append(self._create_event(
            "Luke Skywalker patrols the perimeter of Echo Base",
            "3 ABY",
            "Low",
            participants=[luke["characterID"]],
        ...
        timeline.append(self._create_event(
            "Luke is attacked by a Wampa creature",
            "3 ABY",
            "Medium",
            participants=[luke["characterID"], wampa["characterID"]],
            location=hoth["planetID"]
        ))

        timeline.append(self._create_event(
            "Luke escapes the Wampa cave using the Force",
            "3 ABY",
            "Medium",
            participants=[luke["characterID"]],
            location=hoth["planetID"]
        ...
        timeline.append(self._create_event(
            "Han Solo finds Luke freezing in the Hoth wilderness",
            "3 ABY",
            "High",
            participants=[han["characterID"], luke["characterID"]],
            location=hoth["planetID"],
        ...
        timeline.append(self._create_event(
            "Luke recovers in bacta tank at Echo Base",
            "3 ABY",
            "Low",
            participants=[luke["characterID"],
                          leia["characterID"], han["characterID"]],
            location=echo_base["planetID"]
        ))

        timeline.append(self._create_event(
            "Han Solo and Princess Leia argue about his departure",
            "3 ABY",
            "Medium",
            participants=[han["characterID"], leia["characterID"]],
            location=echo_base["planetID"],
            quote="You could use a good kiss!"
        ))

        timeline.append(self._create_event(
            "Imperial probe droid discovers Rebel base on Hoth",
            "3 ABY",
            "High",
            participants=[],
            location=hoth["planetID"]
        ...
        timeline.append(self._create_event(
            "Admiral Ozzel brings Imperial fleet out of hyperspace too close to Hoth",
            "3 ABY",
            "Medium",
            participants=[vader["characterID"],
                          ozzel["characterID"], piett["characterID"]],
            location=executor["shipID"]
        ))

        timeline.append(self._create_event(
            "Darth Vader executes Admiral Ozzel for his failure",
            "3 ABY",
            "Medium",
            participants=[vader["characterID"],
                          ozzel["characterID"], piett["characterID"]],
            location=executor["shipID"],
            quote="You have failed me for the last time, Admiral."
        ))

        timeline.append(self._create_event(
            "Rebels initiate evacuation of Echo Base",
            "3 ABY",
            "High",
            participants=[leia["characterID"], rebel_officer["characterID"]],
            location=echo_base["planetID"]
        ))

        timeline.append(self._create_event(
            "Imperial forces land on Hoth",
            "3 ABY",
            "High",
            participants=[veers["characterID"]],
            location=hoth["planetID"],
            battle="Battle of Hoth"
        ))

        timeline.append(self._create_event(
            "Rebel snowspeeders engage AT-AT walkers",
            "3 ABY",
            "High",
            participants=[luke["characterID"]],
            location=hoth["planetID"],
        ...
        timeline.append(self._create_event(
            "Luke's snowspeeder is shot down",
            "3 ABY",
            "Medium",
            participants=[luke["characterID"]],
            location=hoth["planetID"],
        ...
        timeline.append(self._create_event(
            "Luke destroys an AT-AT with a thermal detonator",
            "3 ABY",
            "Medium",
            participants=[luke["characterID"]],
            location=hoth["planetID"],
        ...
        timeline.append(self._create_event(
            "Han, Leia, Chewie and C-3PO escape on the Millennium Falcon",
            "3 ABY",
            "High",
            participants=[han["characterID"], leia["characterID"],
                          chewie["characterID"], c3po["characterID"]],
            location=falcon["shipID"]
        ))

        timeline.append(self._create_event(
            "Luke and R2-D2 escape Hoth in X-Wing",
            "3 ABY",
            "Medium",
            participants=[luke["characterID"], r2d2["characterID"]],
            location=x_wing["shipID"]
        ))
        ...
        timeline.append(self._create_event(
            "Luke meets Yoda but doesn't recognize him",
            "3 ABY",
            "High",
            participants=[luke["characterID"],
                          yoda["characterID"], r2d2["characterID"]],
            location=dagobah["planetID"],
            quote="Judge me by my size, do you?"
        ))

        timeline.append(self._create_event(
            "Yoda reveals his identity to Luke",
            "3 ABY",
            "High",
        ...
        timeline.append(self._create_event(
            "Luke enters the Dark Side Cave",
            "3 ABY",
            "High",
        ...
        timeline.append(self._create_event(
            "Luke sees a vision of his friends in pain",
            "3 ABY",
            "High",
            participants=[luke["characterID"], yoda["characterID"]],
            location=dagobah["planetID"]
        ...
        timeline.append(self._create_event(
            "Luke decides to leave Dagobah to save his friends",
            "3 ABY",
            "High",
            participants=[luke["characterID"], yoda["characterID"]],
            location=dagobah["planetID"]
        ...
        # Millennium Falcon sequence
        timeline.append(self._create_event(
            "Millennium Falcon evades Star Destroyers",
            "3 ABY",
            "Medium",
            participants=[han["characterID"], leia["characterID"],
                          chewie["characterID"], c3po["characterID"]],
        ...
        timeline.append(self._create_event(
            "Millennium Falcon hides in an asteroid field",
            "3 ABY",
            "Medium",
            participants=[han["characterID"], leia["characterID"],
                          chewie["characterID"], c3po["characterID"]],
            location=asteroid_field["planetID"],
            quote="Never tell me the odds!"
        ))

        timeline.append(self._create_event(
            "Han and Leia grow closer while hiding in the asteroid cave",
            "3 ABY",
            "Medium",
            participants=[han["characterID"], leia["characterID"]],
            location=falcon["shipID"],
        ...
        timeline.append(self._create_event(
            "Han realizes they're hiding inside a space slug",
            "3 ABY",
            "Medium",
            participants=[han["characterID"], leia["characterID"],
                          chewie["characterID"], c3po["characterID"]],
        ...
        timeline.append(self._create_event(
            "Millennium Falcon escapes the space slug",
            "3 ABY",
            "Medium",
            participants=[han["characterID"], leia["characterID"],
                          chewie["characterID"], c3po["characterID"]],
        ...
        timeline.append(self._create_event(
            "Darth Vader hires bounty hunters to find the Millennium Falcon",
            "3 ABY",
            "Medium",
            participants=[vader["characterID"], boba_fett["characterID"]],
            location=executor["shipID"]
        ))

        timeline.append(self._create_event(
            "Han Solo eludes the Star Destroyers by attaching to the ship",
            "3 ABY",
            "Medium",
        ...
        timeline.append(self._create_event(
            "Captain Needa apologizes to Vader for losing the Millennium Falcon",
            "3 ABY",
            "Low",
            participants=[needa["characterID"], vader["characterID"]],
            location=executor["shipID"],
            quote="Apology accepted, Captain Needa."
        ))

        timeline.append(self._create_event(
            "Boba Fett tracks the Millennium Falcon",
            "3 ABY",
            "Medium",
            participants=[boba_fett["characterID"]],
            location=slave_one["shipID"]
        ))
        ...
        timeline.append(self._create_event(
            ...
            "Medium",
            participants=[han["characterID"], leia["characterID"],
                          chewie["characterID"], c3po["characterID"], lando["characterID"]],
            location=cloud_city["planetID"]
        ))

        timeline.append(self._create_event(
            "Lando Calrissian welcomes Han and friends to Cloud City",
            "3 ABY",
            "Medium",
            participants=[han["characterID"], leia["characterID"],
                          chewie["characterID"], lando["characterID"]],
            location=cloud_city["planetID"],
            quote="How you doin', you old pirate? So good to see you!"
        ))

        timeline.append(self._create_event(
            "C-3PO discovers Imperial stormtrooper components and is shot",
            "3 ABY",
            "Medium",
            participants=[c3po["characterID"]],
            location=cloud_city["planetID"]
        ))

        timeline.append(self._create_event(
            "Darth Vader arrives on Cloud City",
            "3 ABY",
            "High",
            participants=[vader["characterID"], lando["characterID"]],
            location=cloud_city["planetID"]
        ))

        timeline.append(self._create_event(
            "Lando betrays Han Solo and friends to Darth Vader",
            "3 ABY",
            "High",
            participants=[lando["characterID"], han["characterID"],
                          leia["characterID"], chewie["characterID"], vader["characterID"]],
            location=cloud_city["planetID"],
            quote="I had no choice. They arrived right before you did."
        ))
        ...
        timeline.append(self._create_event(
            ...
            "3 ABY",
            "High",
            participants=[vader["characterID"], han["characterID"]],
            location=cloud_city["planetID"]
        ))

        timeline.append(self._create_event(
            "Chewbacca finds dismantled C-3PO",
            "3 ABY",
            "Low",
            participants=[chewie["characterID"], c3po["characterID"]],
            location=cloud_city["planetID"]
        ))

        timeline.append(self._create_event(
            "Darth Vader prepares carbon freezing chamber for Luke",
            "3 ABY",
            "High",
            participants=[vader["characterID"], lando["characterID"]],
            location=cloud_city["planetID"]
        ...
        timeline.append(self._create_event(
            ...
            participants=[han["characterID"], vader["characterID"],
                          leia["characterID"], lando["characterID"], chewie["characterID"]],
            location=cloud_city["planetID"],
            quote="I love you." "I know."
        ))

        timeline.append(self._create_event(
            "Boba Fett takes frozen Han Solo to his ship",
            "3 ABY",
            "Medium",
            participants=[boba_fett["characterID"], han["characterID"]],
            location=cloud_city["planetID"]
        ))

        timeline.append(self._create_event(
            "Lando has change of heart and helps Leia and Chewie escape",
            "3 ABY",
            "High",
            participants=[lando["characterID"], leia["characterID"],
                          chewie["characterID"], r2d2["characterID"], c3po["characterID"]],
            location=cloud_city["planetID"]
        ))

        timeline.append(self._create_event(
            "Luke Skywalker arrives at Cloud City",
            "3 ABY",
            "High",
            participants=[luke["characterID"], r2d2["characterID"]],
            location=cloud_city["planetID"]
        ...
        timeline.append(self._create_event(
            "Luke encounters Boba Fett and stormtroopers",
            "3 ABY",
            "Medium",
            participants=[luke["characterID"], boba_fett["characterID"]],
            location=cloud_city["planetID"]
        ))

        timeline.append(self._create_event(
            "Luke witnesses Han Solo being loaded onto Slave I",
            "3 ABY",
            "Medium",
            participants=[luke["characterID"],
                          han["characterID"], boba_fett["characterID"]],
            location=cloud_city["planetID"]
        ))
        ...
        timeline.append(self._create_event(
            "Darth Vader and Luke Skywalker duel with lightsabers",
            "3 ABY",
            "High",
            participants=[luke["characterID"], vader["characterID"]],
            location=cloud_city["planetID"]
        ...
        timeline.append(self._create_event(
            "Luke chooses to fall rather than join Vader",
            "3 ABY",
            "High",
            participants=[luke["characterID"], vader["characterID"]],
            location=cloud_city["planetID"]
        ...
        timeline.append(self._create_event(
            "Luke calls to Leia through the Force",
            "3 ABY",
            "High",
            participants=[luke["characterID"], leia["characterID"]],
            location=cloud_city["planetID"]
        ...
        timeline.append(self._create_event(
            "Leia senses Luke and directs Lando to turn back for him",
            "3 ABY",
            "High",
            participants=[leia["characterID"],
                          lando["characterID"], chewie["characterID"]],
            location=falcon["shipID"]
        ))

        timeline.append(self._create_event(
            "Luke is rescued from beneath Cloud City",
            "3 ABY",
            "High",
            participants=[luke["characterID"], leia["characterID"],
                          lando["characterID"], chewie["characterID"]],
        ...
        timeline.append(self._create_event(
            "Darth Vader contacts Luke telepathically",
            "3 ABY",
            "Medium",
            participants=[luke["characterID"], vader["characterID"]],
            location=falcon["shipID"]
        ...
        timeline.append(self._create_event(
            "Luke gets cybernetic hand attached to replace lost hand",
            "3 ABY",
            "Medium",
            participants=[luke["characterID"], leia["characterID"]],
            location=falcon["shipID"]
        ...
        timeline.append(self._create_event(
            "Lando and Chewie prepare to search for Han Solo",
            "3 ABY",
            "Medium",
            participants=[lando["characterID"], chewie["characterID"],
                          leia["characterID"], luke["characterID"]],
            location=falcon["shipID"]
        ))

        timeline.append(self._create_event(
            "Rebel fleet regroups as heroes look to the stars",
            "3 ABY",
            "Medium",
            participants=[luke["characterID"], leia["characterID"],
                          r2d2["characterID"], c3po["characterID"]],
            location=falcon["shipID"]
        ))
    ...
    def generate_episode_6_timeline(self) -> List[Dict[str, Any]]:
        """Generate a detailed timeline of events from Episode 6: Return of the Jedi (at least 21 events)"""
        timeline = []
        ...
        c3po = self._find_or_create_droid("C-3PO")
        r2d2 = self._find_or_create_droid("R2-D2")
        admiral_ackbar = self._find_or_create_character("Admiral Ackbar")
        mon_mothma = self._find_or_create_character("Mon Mothma")
        wicket = self._find_or_create_character("Wicket W. Warrick")
        bib_fortuna = self._find_or_create_character("Bib Fortuna")
        gamorrean_guard = self._find_or_create_character("Gamorrean Guard")
        ewoks = self._find_or_create_character("Ewoks")
        rancor = self._find_or_create_character("Rancor")
        moff_jerjerrod = self._find_or_create_character("Moff Jerjerrod")
        ...
        dagobah = self._find_or_create_planet("Dagobah")
        rebel_fleet = self._find_or_create_planet(
            "Rebel Fleet Assembly Point", "Space", "Sullust")
        sarlacc_pit = self._find_or_create_planet(
            "Great Pit of Carkoon", "Desert", "Tatoo")
        ewok_village = self._find_or_create_planet(
            "Ewok Village", "Forest", "Endor")
        endor_bunker = self._find_or_create_planet(
            "Imperial Bunker", "Forest", "Endor")

        # Key ships
        falcon = self._find_or_create_ship("Millennium Falcon")
        executor = self._find_or_create_ship("Executor")
        shuttle_tydirium = self._find_or_create_ship(
            "Imperial Shuttle Tydirium")
        home_one = self._find_or_create_ship("Home One")
        x_wing = self._find_or_create_ship("X-Wing Red Five")
        sail_barge = self._find_or_create_ship("Jabba's Sail Barge")

        # Timeline events - continuing with the insanity inducing drudgery of adding more timeline events...
        # silent screams...
        # deep breaths
        # carry on...
        ...
        timeline.append(self._create_event(
            "C-3PO and R2-D2 arrive at Jabba's Palace",
            "4 ABY",
            "Medium",
            participants=[c3po["characterID"],
                          r2d2["characterID"], bib_fortuna["characterID"]],
            location=jabba_palace["planetID"]
        ))

        timeline.append(self._create_event(
            "R2-D2 plays Luke's message to Jabba the Hutt",
            "4 ABY",
            "Medium",
            participants=[r2d2["characterID"],
                          c3po["characterID"], jabba["characterID"]],
            location=jabba_palace["planetID"]
        ))

        timeline.append(self._create_event(
            "C-3PO is forced to become Jabba's interpreter",
            "4 ABY",
            "Low",
            participants=[c3po["characterID"], jabba["characterID"]],
            location=jabba_palace["planetID"]
        ))

        timeline.append(self._create_event(
            "Bounty hunter 'Boushh' (Leia in disguise) arrives with Chewbacca",
            "4 ABY",
            "Medium",
            participants=[leia["characterID"],
                          chewie["characterID"], jabba["characterID"]],
            location=jabba_palace["planetID"],
            quote="Yotō. Yotōkō."
        ))

        timeline.append(self._create_event(
            "Leia frees Han Solo from carbonite",
            "4 ABY",
            "High",
            participants=[leia["characterID"], han["characterID"]],
            location=jabba_palace["planetID"]
        ))

        timeline.append(self._create_event(
            "Jabba captures Leia and Han",
            "4 ABY",
            "High",
            participants=[jabba["characterID"],
                          leia["characterID"], han["characterID"]],
            location=jabba_palace["planetID"]
        ))

        timeline.append(self._create_event(
            "Jabba forces Leia to become his slave",
            "4 ABY",
            "Medium",
            participants=[jabba["characterID"], leia["characterID"]],
            location=jabba_palace["planetID"]
        ))

        timeline.append(self._create_event(
            "Luke Skywalker arrives at Jabba's Palace",
            "4 ABY",
            "High",
            participants=[luke["characterID"],
                          jabba["characterID"], bib_fortuna["characterID"]],
            location=jabba_palace["planetID"],
            quote="You will bring Captain Solo and the Wookiee to me."
        ))
        ...
        timeline.append(self._create_event(
            ...
            "Medium",
            participants=[luke["characterID"],
                          jabba["characterID"], rancor["characterID"]],
            location=jabba_palace["planetID"]
        ))
        ...
        timeline.append(self._create_event(
            ...
            "4 ABY",
            "Medium",
            participants=[luke["characterID"], rancor["characterID"]],
            location=jabba_palace["planetID"]
        ))

        timeline.append(self._create_event(
            "Jabba sentences Luke, Han, and Chewie to death at the Sarlacc pit",
            "4 ABY",
            "Medium",
            participants=[jabba["characterID"], luke["characterID"],
                          han["characterID"], chewie["characterID"]],
            location=jabba_palace["planetID"]
        ))

        timeline.append(self._create_event(
            "Luke, Han and Chewie are taken to the Great Pit of Carkoon",
            "4 ABY",
            "Medium",
            participants=[luke["characterID"], han["characterID"], chewie["characterID"],
                          leia["characterID"], c3po["characterID"], r2d2["characterID"], jabba["characterID"]],
            location=sarlacc_pit["planetID"]
        ))

        timeline.append(self._create_event(
            "R2-D2 launches Luke's lightsaber to him",
            "4 ABY",
            "High",
            participants=[luke["characterID"], r2d2["characterID"]],
            location=sarlacc_pit["planetID"]
        ...
        timeline.append(self._create_event(
            ...
            "High",
            participants=[luke["characterID"], han["characterID"], chewie["characterID"],
                          leia["characterID"], lando["characterID"], boba_fett["characterID"], jabba["characterID"]],
            location=sarlacc_pit["planetID"]
        ))
        ...
        timeline.append(self._create_event(
            "Leia strangles Jabba the Hutt with her chain",
            "4 ABY",
            "High",
            participants=[leia["characterID"], jabba["characterID"]],
            location=sail_barge["shipID"]
        ))

        timeline.append(self._create_event(
            "Heroes destroy Jabba's sail barge and escape",
            "4 ABY",
            "Medium",
            participants=[luke["characterID"], han["characterID"], leia["characterID"],
                          chewie["characterID"], lando["characterID"], r2d2["characterID"], c3po["characterID"]],
            location=sail_barge["shipID"]
        ))
        ...
        timeline.append(self._create_event(
            "Luke finds Yoda dying",
            "4 ABY",
            "High",
            participants=[luke["characterID"], yoda["characterID"]],
            location=dagobah["planetID"],
            quote="When gone am I, the last of the Jedi will you be."
        ))
        ...
        timeline.append(self._create_event(
            "Obi-Wan's spirit tells Luke that Leia is his twin sister",
            "4 ABY",
            "High",
            participants=[luke["characterID"]],
            location=dagobah["planetID"],
            quote="The other that Yoda spoke of is your twin sister."
        ))

        # Rebel Fleet sequence
        timeline.append(self._create_event(
            "Rebel leaders gather to plan attack on Death Star II",
            "4 ABY",
            "High",
            participants=[mon_mothma["characterID"], admiral_ackbar["characterID"],
                          leia["characterID"], han["characterID"], chewie["characterID"], c3po["characterID"]],
            location=home_one["shipID"]
        ))

        timeline.append(self._create_event(
            "Mon Mothma reveals that Emperor Palpatine is on the Death Star",
            "4 ABY",
            "High",
            participants=[mon_mothma["characterID"],
                          admiral_ackbar["characterID"]],
            location=home_one["shipID"]
        ))

        timeline.append(self._create_event(
            "Han Solo volunteers to lead ground assault on shield generator",
            "4 ABY",
            "Medium",
            participants=[han["characterID"], leia["characterID"],
                          chewie["characterID"], luke["characterID"]],
            location=home_one["shipID"]
        ))
        ...
        timeline.append(self._create_event(
            ...
            quote="The Force is strong in my family. My father has it. I have it. And... my sister has it."
        ))

        # Endor sequence
        timeline.append(self._create_event(
            "Imperial Shuttle Tydirium approaches Endor with stolen codes",
            "4 ABY",
            "Medium",
            participants=[han["characterID"], luke["characterID"], leia["characterID"],
                          chewie["characterID"], c3po["characterID"], r2d2["characterID"]],
            location=shuttle_tydirium["shipID"]
        ))

        timeline.append(self._create_event(
            "Vader senses Luke's presence on the shuttle",
            "4 ABY",
            "Medium",
            participants=[vader["characterID"], luke["characterID"]],
            location=executor["shipID"],
            quote="I'm endangering the mission. I shouldn't have come."
        ))

        timeline.append(self._create_event(
            "Rebels land on Endor",
            "4 ABY",
            "Medium",
            participants=[han["characterID"], luke["characterID"], leia["characterID"],
                          chewie["characterID"], c3po["characterID"], r2d2["characterID"]],
        ...
        timeline.append(self._create_event(
            "Scout troopers discover the Rebels and Leia becomes separated",
            "4 ABY",
            "Medium",
            participants=[luke["characterID"], han["characterID"],
                          leia["characterID"], chewie["characterID"]],
        ...
        timeline.append(self._create_event(
            "Leia meets Wicket the Ewok",
            "4 ABY",
            "Medium",
            participants=[leia["characterID"], wicket["characterID"]],
            location=endor["planetID"]
        ))

        timeline.append(self._create_event(
            "Luke, Han, Chewie, and the droids are captured by Ewoks",
            "4 ABY",
            "Medium",
            participants=[luke["characterID"], han["characterID"], chewie["characterID"],
                          c3po["characterID"], r2d2["characterID"], ewoks["characterID"]],
            location=endor["planetID"]
        ))

        timeline.append(self._create_event(
            "Ewoks worship C-3PO as a god",
            "4 ABY",
            "Medium",
            participants=[c3po["characterID"], ewoks["characterID"],
                          luke["characterID"], han["characterID"], chewie["characterID"]],
            location=ewok_village["planetID"],
            quote="It's against my programming to impersonate a deity."
        ))

        timeline.append(self._create_event(
            "Luke demonstrates the Force to free his friends",
            "4 ABY",
            "Medium",
            participants=[luke["characterID"],
                          c3po["characterID"], ewoks["characterID"]],
            location=ewok_village["planetID"]
        ))

        timeline.append(self._create_event(
            "C-3PO tells the Ewoks the story of the Rebellion",
            "4 ABY",
            "Medium",
            participants=[c3po["characterID"], ewoks["characterID"],
                          luke["characterID"], han["characterID"], leia["characterID"]],
            location=ewok_village["planetID"]
        ))

        timeline.append(self._create_event(
            "Ewoks agree to help the Rebels fight the Empire",
            "4 ABY",
            "Medium",
            participants=[c3po["characterID"], ewoks["characterID"], luke["characterID"],
                          han["characterID"], leia["characterID"], chewie["characterID"]],
            location=ewok_village["planetID"]
        ))

        timeline.append(self._create_event(
            "Luke tells Leia he must confront Vader",
            "4 ABY",
            "High",
            participants=[luke["characterID"], leia["characterID"]],
            location=ewok_village["planetID"],
        ...
        timeline.append(self._create_event(
            "Luke surrenders to Imperial troops on Endor",
            "4 ABY",
            "High",
            participants=[luke["characterID"], vader["characterID"]],
            location=endor["planetID"]
        ...
        timeline.append(self._create_event(
            "Luke and Vader meet face to face",
            "4 ABY",
            "High",
            participants=[luke["characterID"], vader["characterID"]],
            location=endor["planetID"],
            quote="I see you have constructed a new lightsaber."
        ))

        timeline.append(self._create_event(
            "Luke is taken to the Death Star to meet the Emperor",
            "4 ABY",
            "High",
            participants=[luke["characterID"], vader["characterID"]],
        ...
        timeline.append(self._create_event(
            ...
            "4 ABY",
            "High",
            participants=[luke["characterID"], leia["characterID"], han["characterID"], chewie["characterID"],
                          lando["characterID"], admiral_ackbar["characterID"], ewoks["characterID"]],
            location=endor["planetID"],
            battle="Battle of Endor"
        ))

        timeline.append(self._create_event(
            "Rebels and Ewoks attack Imperial bunker",
            "4 ABY",
            "High",
            participants=[han["characterID"], leia["characterID"],
                          chewie["characterID"], ewoks["characterID"]],
            location=endor_bunker["planetID"],
            battle="Battle of Endor"
        ))

        timeline.append(self._create_event(
            "Rebel fleet arrives at Endor to find the shield still up",
            "4 ABY",
            "High",
            participants=[lando["characterID"], admiral_ackbar["characterID"]],
            location=home_one["shipID"],
            battle="Battle of Endor",
            quote="It's a trap!"
        ))
        ...
        timeline.append(self._create_event(
            "Imperial fleet ambushes the Rebel fleet",
            "4 ABY",
            "High",
            participants=[lando["characterID"], admiral_ackbar["characterID"]],
            location=home_one["shipID"],
        ...
        timeline.append(self._create_event(
            "Leia is wounded during the Endor battle",
            "4 ABY",
            "Medium",
            participants=[leia["characterID"], han["characterID"]],
            location=endor["planetID"],
        ...
        timeline.append(self._create_event(
            "Han and Leia capture the bunker",
            "4 ABY",
            "High",
            participants=[han["characterID"],
                          leia["characterID"], chewie["characterID"]],
        ...
        timeline.append(self._create_event(
            "Rebels destroy the shield generator",
            "4 ABY",
            "High",
            participants=[han["characterID"],
                          leia["characterID"], chewie["characterID"]],
        ...
        # Death Star II sequence
        timeline.append(self._create_event(
            "Luke confronts Emperor Palpatine",
            "4 ABY",
            "High",
            participants=[luke["characterID"],
                          vader["characterID"], emperor["characterID"]],
            location=death_star2["planetID"],
            quote="Welcome, young Skywalker. I have been expecting you."
        ))

        timeline.append(self._create_event(
            "Emperor reveals that the Rebel attack is a trap",
            "4 ABY",
            "High",
            participants=[emperor["characterID"], luke["characterID"]],
            location=death_star2["planetID"]
        ))

        timeline.append(self._create_event(
            "Emperor goads Luke to take his lightsaber and strike him down",
            "4 ABY",
            "High",
            participants=[emperor["characterID"],
                          luke["characterID"], vader["characterID"]],
            location=death_star2["planetID"],
            quote="Strike me down with all your hatred and your journey towards the dark side will be complete."
        ))

        timeline.append(self._create_event(
            "Luke fights Vader in the Emperor's throne room",
            "4 ABY",
            "High",
            participants=[luke["characterID"],
                          vader["characterID"], emperor["characterID"]],
        ...
        timeline.append(self._create_event(
            ...
            participants=[luke["characterID"], vader["characterID"]],
            location=death_star2["planetID"],
            quote="If you will not turn to the dark side, then perhaps she will."
        ))

        timeline.append(self._create_event(
            "Luke attacks Vader in anger",
            "4 ABY",
            "High",
            participants=[luke["characterID"], vader["characterID"]],
            location=death_star2["planetID"]
        ...
        timeline.append(self._create_event(
            "Luke defeats Vader and cuts off his hand",
            "4 ABY",
            "High",
            participants=[luke["characterID"],
                          vader["characterID"], emperor["characterID"]],
        ...
        timeline.append(self._create_event(
            "Luke refuses to kill Vader and join the dark side",
            "4 ABY",
            "High",
            participants=[luke["characterID"],
                          vader["characterID"], emperor["characterID"]],
        ...
        timeline.append(self._create_event(
            "Emperor attacks Luke with Force lightning",
            "4 ABY",
            "High",
            participants=[emperor["characterID"],
                          luke["characterID"], vader["characterID"]],
        ...
        timeline.append(self._create_event(
            "Vader turns against the Emperor and saves Luke",
            "4 ABY",
            "High",
            participants=[luke["characterID"],
                          vader["characterID"], emperor["characterID"]],
        ...
        timeline.append(self._create_event(
            "Vader throws the Emperor down a reactor shaft",
            "4 ABY",
            "High",
            participants=[vader["characterID"], emperor["characterID"]],
            location=death_star2["planetID"]
        ))

        timeline.append(self._create_event(
            "Lando leads fighters into the Death Star's core",
            "4 ABY",
            "High",
        ...
        timeline.append(self._create_event(
            "Dying Vader asks Luke to remove his mask",
            "4 ABY",
            "High",
            participants=[luke["characterID"], vader["characterID"]],
            location=death_star2["planetID"],
            quote="Luke, help me take this mask off."
        ))

        timeline.append(self._create_event(
            "Darth Vader dies after returning to the light side",
            "4 ABY",
            "High",
            participants=[luke["characterID"], vader["characterID"]],
            location=death_star2["planetID"],
            quote="You were right about me. Tell your sister... you were right."
        ))

        timeline.append(self._create_event(
            "Luke escapes the Death Star with Vader's body",
            "4 ABY",
            "High",
            participants=[luke["characterID"], vader["characterID"]],
            location=shuttle_tydirium["shipID"]
        ...
        timeline.append(self._create_event(
            "Rebels destroy the Death Star's main reactor",
            "4 ABY",
            "High",
            participants=[lando["characterID"]],
            location=death_star2["planetID"],
        ...
        timeline.append(self._create_event(
            "Luke burns Vader's armor in a funeral pyre on Endor",
            "4 ABY",
            "High",
            participants=[luke["characterID"]],
            location=endor["planetID"]
        ...
        timeline.append(self._create_event(
            "Celebration of the fall of the Empire across the galaxy",
            "4 ABY",
            "High",
            participants=[luke["characterID"], leia["characterID"], han["characterID"], lando["characterID"],
                          chewie["characterID"], c3po["characterID"], r2d2["characterID"], ewoks["characterID"]],
            location=endor["planetID"]
        ))

        timeline.append(self._create_event(
            "Luke sees the spirits of Obi-Wan, Yoda, and Anakin Skywalker",
            "4 ABY",
            "High",
            participants=[luke["characterID"]],
            location=endor["planetID"]
...
if __name__ == "__main__":
    ...
    data_dict = generate_data_dictionary()
    serialize_to_json(data_dict, "star_wars_data_dictionary.json")
...
# bro this was supposed to be fun...
# next time define fun, before you lose your shit.
# numbskull.
