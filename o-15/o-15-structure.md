...
Class: WaterSystem          
  - systemID: string          
  - region: string          
  - waterQualityIndex: float        // new data property          
  - overallCapacity: float          // new data property          
  - operationalStatus: string       // new data property          
...
Relationship: hasReservoir (WaterSystem → Reservoir, 1..*)          
Relationship: containsDam (Reservoir → Dam, 0..1)          
Relationship: flowsThrough (Dam → Canal, 0..*)          
Relationship: housesPumpStation (Canal → PumpStation, 0..*)          
Relationship: powersTurbine (Dam → HydroelectricTurbine, 0..*)          
Relationship: suppliesPlant (Canal → WaterTreatmentPlant, 0..*)          
Relationship: measuresQuality (WaterTreatmentPlant → WaterQualitySensor, 0..*)          
Relationship: triggersEvent (WaterQualitySensor → AlertEvent, 0..*)          
Relationship: governedBy (Dam → RegulatoryPermit, 0..*)          
Relationship: referencesPlan (GridMaintenance → TransmissionLine, 0..*) // Not in original #15—typo? We'll omit.          
          
Relationship: linksPipeline (WaterTreatmentPlant → DistributionPipeline, 0..*)          
Relationship: usedInSimulation (FloodSimulation → Dam, 0..*)          
Relationship: warnsSystem (AlertEvent → WaterSystem, 0..1)          
          
// New object properties:          
          
Relationship: auditsPermit (RegulatoryPermit → WaterSystem, 0..1)            
// e.g., the permit is audited/renewed by an overseeing waterSystem context          
          
Relationship: includesTurbine (WaterSystem → HydroelectricTurbine, 0..*)            
// the entire water system might track multiple turbines directly          
          
Relationship: referencesMaintenancePlan (FloodSimulation → MaintenancePlan, 0..*)            
// a flood simulation might factor in planned maintenance schedules          
