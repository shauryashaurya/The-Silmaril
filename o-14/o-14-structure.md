...
Class: DistributionNetwork          
  - networkID: string          
  - region: string          
  - primaryMode: string        // new data property          
  - averageDistanceCovered: float  // new data property          
          
Class: Warehouse          
  - warehouseID: string          
  - capacity: float          
  - location: string          
  - temperatureControlled: boolean  // new data property          
...
Relationship: containsWarehouse (DistributionNetwork → Warehouse, 1..*)          
Relationship: hasTransportHub (DistributionNetwork → TransportHub, 0..*)          
Relationship: usesVehicle (TransportHub → Vehicle, 1..*)          
Relationship: assignedRoute (Vehicle → Route, 0..1)          
Relationship: scheduledDelivery (DeliverySchedule → Route, 0..1)          
Relationship: holdsInventory (Warehouse → InventoryItem, 0..*)          
Relationship: groupedInto (InventoryItem → PackingUnit, 0..*)          
Relationship: drivenBy (Vehicle → Driver, 0..1)          
Relationship: createsShipment (Warehouse → Shipment, 0..*)          
Relationship: governedBy (Shipment → SupplyContract, 0..1)          
Relationship: integratedWith (FleetManagementSystem → Vehicle, 0..*)          
Relationship: referencesSchedule (Shipment → DeliverySchedule, 0..1)          
          
// New object properties:          
Relationship: regulatedBy (DistributionNetwork → SupplyContract, 0..*)           
// e.g., entire network might have a supply contract context          
          
Relationship: managesWarehouse (FleetManagementSystem → Warehouse, 0..*)           
// a fleet management system might also manage certain warehouses          
