...
class OntologyReasoner:
    ...
    def load_ontology(self):
        try:
            ...
            print(f"N3 parsing failed: {e}")
            try:
                # Try Turtle format as fallback
                self.graph.parse(self.ontology_path, format='turtle')
            except Exception as e2:
                print(f"Turtle parsing failed: {e2}")
                # Manual parsing for comma-separated class declarations
                with open(self.ontology_path, 'r', encoding='utf-8') as f:
                    content = f.read()

                # Fix comma-separated class declarations
                content = re.sub(
                    r'(:[\w,\s:]+)\s+a\s+rdfs:Class\s*\.',
                    lambda m: '\n'.join([f"{cls.strip()} a rdfs:Class ."
                                         for cls in m.group(1).split(',') if cls.strip()]),
                    content
                )

                # Write corrected content to temporary file
                temp_file = self.ontology_path + '.temp'
                with open(temp_file, 'w', encoding='utf-8') as f:
                    f.write(content)

                # Parse corrected file
                self.graph.parse(temp_file, format='turtle')
    ...
    def check_sensor_data_freshness(self, data_loaded):
        ...
        sensor_df = data_loaded['Sensor']
        if 'lastUpdateTime' not in sensor_df.columns:
            print("Warning: lastUpdateTime column not found in Sensor data")
            return
        ...
        for _, row in sensor_df.iterrows():
            if pd.notna(row['lastUpdateTime']):
                try:
                    ...
                    print(
                        f"Warning: Invalid date format for sensor {row.get('sensorID')}")
    ...
    def check_zone_capacity_violations(self, data_loaded):
        ...
        if 'occupancyCapacity' not in zone_df.columns:
            print("Warning: occupancyCapacity column not found in Zone data")
            return

        zone_capacity = dict(
            zip(zone_df['zoneID'], zone_df['occupancyCapacity']))
    ...
    def check_energy_waste_detection(self, data_loaded):
        ...
        if 'zoneID' in occupant_df.columns:
            occupied_zones = set(occupant_df['zoneID'].dropna())
        else:
            occupied_zones = set()
