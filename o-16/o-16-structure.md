...
Class: Hospital          
  ...
  - name: string          
  - bedCapacity: int          
  - hospitalRating: float            // new data property          
  - emergencyCapacity: int           // new data property          
...
Class: Physician          
  ...
  - yearsOfExperience: int           // new data property          
...
Class: MedicalRecord          
  ...
  - recordDate: dateTime          
  - notes: string          
  - diagnosisCode: string            // new data property          
...
Class: InsurancePlan          
  ...
  - planName: string          
  - coverageType: string          
  - planPremium: float               // new data property          
...
Relationship: includesHospital (MedicalNetwork → Hospital, 1..*)          
Relationship: hasClinic (MedicalNetwork → Clinic, 0..*)          
Relationship: organizesDept (Hospital → MedicalDepartment, 0..*)          
Relationship: employsPhysician (MedicalDepartment → Physician, 0..*)          
Relationship: schedulesAppointment (Clinic → Appointment, 0..*)          
Relationship: visitedBy (Appointment → Patient, 1..*)          
Relationship: yieldsRecord (Appointment → MedicalRecord, 0..1)          
Relationship: requiresLabTest (MedicalRecord → LabTest, 0..*)          
Relationship: billedTo (MedicalRecord → BillingAccount, 0..1)          
Relationship: coveredBy (BillingAccount → InsurancePlan, 0..1)          
Relationship: dispensedFrom (Pharmacy → Prescription, 0..*)          
Relationship: assignedTo (Prescription → Patient, 1..*)          
Relationship: enrollsStudy (ClinicalStudy → Patient, 0..*)          
Relationship: authoredBy (LabTest → Physician, 0..1)          
          
// New object properties:          
Relationship: coManagedBy (Hospital → Physician, 0..*)              
  // e.g., multiple lead physicians co-manage a hospital          
          
Relationship: preferredPharmacy (Patient → Pharmacy, 0..1)            
  // each patient can choose one main pharmacy          
          
Relationship: coFunds (InsurancePlan → Hospital, 0..*)                 
  // an insurance plan may partially fund certain hospitals          
          
Relationship: referencesDiagnosis (MedicalRecord → MedicalDepartment, 0..1)            
  // a record referencing the department that handled its main diagnosis          
          
Relationship: delegatesTo (Physician → Physician, 0..*)                 
  // a senior physician can delegate tasks to a junior physician          
