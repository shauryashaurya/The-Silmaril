...
# Configure logging
logging.basicConfig(filename='./supply_chain_reasoner_usage.log', level=logging.INFO,
...
class SupplyChainAnalyticsEngine:
    ...
    def __init__(self, data_location: str = './data', output_location: str = './'):
    ...
    def export_json_report(self, filename: Optional[str] = None) -> str:
        ...
        if not filename:
            filename = f"supply_chain_analysis.json"
    ...
    **Expected Benefit**: {rec['expected_benefit']}
    ...
    Evaluated {sum(stats['entity_counts'].values())} entities across the supply chain network. {self.business_insights['supply_chain_health']['overall_health_score']:.0f}% health score.

    **Possible Actions**: {len([rec for rec in self.strategic_recommendations if rec['priority'] == 'HIGH'])} high-priority actionable recommendations.
    ...
    **Risk Mitigation**: Address some risks to prevent potential exposure of ${self.executive_summary['financial_impact']['risk_exposure']:,.2f}.
