...
Class: System          
  - systemID: string          
  - systemName: string          
  - systemVersion: string                 // new data property          
  - operationalRegion: string             // new data property          
...
Relationship: hasSubsystem (System → Subsystem, 1..*)          
Relationship: hasComponent (Subsystem → Component, 0..*)          
Relationship: monitoredBy (Component → Sensor, 0..*)          
Relationship: emitsSignal (Sensor → Signal, 0..*)          
Relationship: assignedTo (MaintenanceOrder → Technician, 1..*)          
Relationship: triggersAlert (Signal → Alert, 0..1)          
Relationship: usesFailureMode (SimulationProfile → FailureMode, 0..*)          
Relationship: requiresMaintenance (Component → MaintenanceOrder, 0..*)          
Relationship: referencesProfile (Alert → SimulationProfile, 0..1)          
Relationship: identifiesMode (Alert → FailureMode, 0..1)          
          
// New object properties          
Relationship: governedBy (System → Technician, 0..1)           
// e.g., some Technician is overall system manager          
          
Relationship: referencesSubsystem (FailureMode → Subsystem, 0..1)           
// a specific failure mode might primarily affect a certain subsystem          
